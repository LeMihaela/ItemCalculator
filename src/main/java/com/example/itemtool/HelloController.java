...
public class HelloController {
    ...
    @FXML
    private Button saveToFile = new Button();
    ...
    public void setSaveToFile() throws IOException {
        FileWriter a = new FileWriter("items.txt");
        Item[] currentItem = new Item[6];
        for (int j = 0; j < currentItem.length ; j++) {
            for (int i = 0; i < itemList.size(); i++) {
                if(selected[j] != null){
        ...
        a.write(lines[0] + lines[1] +lines[2] +lines[3] + lines[4] + lines[5]
                + sumLine + sumStringBase+ sumStringBonus1 + sumStringBonus2);
        a.close();
    }
}